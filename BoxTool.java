...
public class BoxTool implements Brush {
	...
	public void draw() {
		...
		Frame.canvas.setOnMouseReleased(new EventHandler<MouseEvent>(){
			...
			public void handle(MouseEvent e){
				...
				y2 = e.getY();
				if (x1<x2&&y1<y2)
					Frame.gc.fillRect(x1, y1, Math.abs(x2-x1), Math.abs(y2-y1));
				else if (x2<x1&&y1<y2)
					Frame.gc.fillRect(x2, y1, Math.abs(x2-x1), Math.abs(y2-y1));
				else if (x2<x1&&y2<y1)
					Frame.gc.fillRect(x2, y2, Math.abs(x2-x1), Math.abs(y2-y1));
				else if (x1<x2&&y2<y1)
					Frame.gc.fillRect(x1, y2, Math.abs(x2-x1), Math.abs(y2-y1));
			}
		});
